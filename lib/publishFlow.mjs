...
export async function runPreflight({ supabase, user, articleId }) {
  ...
  if (editorialManaged) {
    ...
    const hybridReady =
      ...
      Number(draft.image_qa?.editorial_quality || 0) >= 8 &&
      Number(draft.image_qa?.rev_environment_consistency || 0) >= 8 &&
      Number(draft.image_qa?.brand_space_authenticity || 0) >= 8 &&
      draft.image_qa?.source_material_scope_pass === true &&
      draft.image_qa?.unknown_trainer_present === false &&
      draft.image_qa?.non_customer_people_present === false &&
      draft.image_qa?.customer_only_or_no_people === true;

    const imageReady = sourceLockReady || hybridReady;

    if (imageReady) {
      const modeLabel = hybridReady
        ? 'GPT Image hybrid / THE REV. environment + no unknown trainer'
        : 'supplied photo locked / exact typography';
      checks.push(check(
        'image_release',
        '記事画像',
        CHECK_OK,
      ...
      const msg = '記事画像の公開QCが未完了です。versioned Thumbnail / OGP、記事関連性、シリーズ整合、THE REV.環境整合、未知トレーナー不在などのQCを満たすまで公開できません。';
