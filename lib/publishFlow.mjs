...
import { HYBRID_IMAGE_FORMAT, hybridQaReady } from './editorialHybridImageFormat.mjs';
...
export async function runPreflight({ supabase, user, articleId }) {
  ...
  if (editorialManaged) {
    ...
    const sourceLockReady =
      commonImageReady &&
      draft.image_render_version === IMAGE_RENDER_VERSION &&
      String(draft.image_strategy || '').startsWith('reference-v2-source-lock-') &&
      Number(draft.image_qa?.source_identity_preservation || 0) >= 9 &&
      draft.image_qa?.invented_people_or_objects === false &&
      draft.image_qa?.source_photo_changed_materially === false;

    const imageReview = evaluateEditorialImageReview({ draft, qa: draft.image_qa || {} });

    checks.push(check(
      'image_review_gate',
      '画像Review Gate',
      imageReview.ok ? CHECK_OK : CHECK_ERROR,
      imageReview.ok
        ? `${imageReview.mode} / trainer absent / generated customer / recent-4 reuse check PASS。`
        : imageReview.errors.join(' ')
    ));

    if (!imageReview.ok) {
      const gateMessage = `画像Review Gate未通過: ${imageReview.errors.join(' ')}`;
      blocker = blocker || { code: 'image_review_gate_failed', status: 409, message: gateMessage };
    }
    ...
    // V2.4 policy: new/re-reviewed Editorial AI articles are publishable only
    // through the generated Hybrid route. Source-lock remains diagnostic only.
    const sourceLockReviewReady = false;
    const imageReady = hybridReady;

    if (imageReady) {
      const modeLabel = `GPT Image hybrid / ${HYBRID_IMAGE_FORMAT.policyRevision} / ${HYBRID_IMAGE_FORMAT.layoutTemplateId} / generated customer + real THE REV. environment`;
      checks.push(check(
        'image_release',
        '記事画像',
        CHECK_OK,
        `旧5記事参照 ${IMAGE_STYLE_TEMPLATE} / ${modeLabel} / Brand QA PASS。`
      ));
    } else {
      const msg = '記事画像の公開QCが未完了です。Hybrid生成、トレーナー不在、顧客のみ/人物なし、THE REV.実背景、直近4記事との画像・背景重複なし、versioned Thumbnail / OGP、ブランドQCをすべて満たすまで公開できません。';
...
export function draftSummary(draft) {
  ...
  return {
    ...
    image_source_path: draft.image_source_path,
    ...
    gbp_image: draft.gbp_image,
    gbp_image_status: draft.gbp_image_status,
    gbp_image_checked_at: draft.gbp_image_checked_at,
    gbp_image_qa: draft.gbp_image_qa,
    gbp_image_asset_version: draft.gbp_image_asset_version,
    gbp_image_last_error: draft.gbp_image_last_error,
    gbp_image_attempts: draft.gbp_image_attempts
  };
}
