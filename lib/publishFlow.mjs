...
export async function runPreflight({ supabase, user, articleId }) {
  ...
  if (editorialManaged) {
    ...
    checks.push(check(
      ...
      imageReview.ok
        ? `${imageReview.mode} / trainer absent / customer-only-or-no-people / recent-4 reuse check PASS。`
    ...
    const sourceLockReviewReady =
      sourceLockReady &&
      imageReview.ok;

    const imageReady = sourceLockReviewReady || hybridReady;

    if (imageReady) {
      const modeLabel = hybridReady
        ? `GPT Image hybrid / ${HYBRID_IMAGE_FORMAT.id} / THE REV. environment + no unknown trainer`
        : 'explicit source-lock fallback / exact typography';
      checks.push(check(
        'image_release',
        '記事画像',
        CHECK_OK,
