...
import { GithubError, checkConnection, getFile, fileExists, publicGithubConfig } from './githubContent.mjs';
import { IMAGE_RENDER_VERSION } from './editorialImage.mjs';
...
export async function runPreflight({ supabase, user, articleId }) {
  ...
  if (editorialManaged) {
    ...
    if (imageReady) {
      checks.push(check('image_release', '記事画像', CHECK_OK, `文字入り統一デザイン画像 READY（${IMAGE_RENDER_VERSION}）。`));
    } else {
      const msg = '文字入りサムネイル / OGP の最終生成が完了していません。画像が揃うまで公開できません。';
  ...
  try {
    ...
    if (editorialManaged && !blocker) {
      ...
      if (!thumbPath || !ogPath) {
        ...
        const [thumbOk, ogOk] = await Promise.all([fileExists(thumbPath), fileExists(ogPath)]);
        if (thumbOk && ogOk) {
          checks.push(check('image_assets', '画像ファイル実在', CHECK_OK, 'Thumbnail / OGP のGitHub実体を確認しました。'));
        } else {
          const msg = `画像ファイルが不足しています（Thumbnail: ${thumbOk ? 'OK' : 'missing'} / OGP: ${ogOk ? 'OK' : 'missing'}）。`;
          checks.push(check('image_assets', '画像ファイル実在', CHECK_ERROR, msg));
          blocker = blocker || { code: 'image_asset_missing', status: 409, message: msg };
        }
      }
