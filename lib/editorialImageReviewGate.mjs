...
export const EDITORIAL_REALISM_QC_VERSION = 'v1';
export const EDITORIAL_REALISM_QC_CUTOVER = '2026-09-24T01:00:00.000Z';
export const EDITORIAL_SEMANTIC_QC_CUTOVER = '2026-09-26T00:00:00.000Z';
...
export function evaluateEditorialImageReview({ draft = {}, qa = draft?.image_qa || {}, recentHistory = [] } = {}) {
  ...
  add(
    'manual_visual_rejection_absent',
    qa?.manual_visual_rejection !== true,
    '人間Visual QCでREJECTされています。自動QAの点数より人間Reviewを優先します。'
  );

  const qaCheckedAt = Date.parse(clean(qa.checked_at));
  const realismCutover = Date.parse(EDITORIAL_REALISM_QC_CUTOVER);
  const realismRequired =
    !legacyAcceptedReference &&
    Number.isFinite(qaCheckedAt) &&
    qaCheckedAt >= realismCutover;

  if (realismRequired) {
    add(
      'realism_qc_version',
      clean(qa.realism_qc_version) === EDITORIAL_REALISM_QC_VERSION,
      `realism_qc_version は ${EDITORIAL_REALISM_QC_VERSION} である必要があります。`
    );
    add('human_environment_integration', Number(qa.human_environment_integration || 0) >= 8, '人物と実背景の統合品質が8未満、または未記録です。');
    add('perspective_scale_consistency', Number(qa.perspective_scale_consistency || 0) >= 8, '人物の縮尺・遠近整合が8未満、または未記録です。');
    add('ground_contact_shadow_consistency', Number(qa.ground_contact_shadow_consistency || 0) >= 8, '床接地・接触影の整合が8未満、または未記録です。');
    add('lighting_consistency', Number(qa.lighting_consistency || 0) >= 8, '人物と背景の光・色温度整合が8未満、または未記録です。');
    add('anatomy_pose_realism', Number(qa.anatomy_pose_realism || 0) >= 8, '人物の解剖・姿勢リアリズムが8未満、または未記録です。');
    add('no_cutout_or_sticker_look', boolIs(qa.no_cutout_or_sticker_look, true), '人物が切り抜き/ステッカー合成に見えます、または判定が未記録です。');
    add('location_semantics_pass', boolIs(qa.location_semantics_pass, true), '人物の配置が店舗利用として不自然です、または判定が未記録です。');
    add('exercise_pose_plausible', boolIs(qa.exercise_pose_plausible, true), '運動姿勢が不自然です、または判定が未記録です。');
  } else {
    checks.realism_qc_legacy_pre_cutover = true;
  }
  ...
  const semanticCheckedAt = Date.parse(clean(qa.checked_at));
  const semanticRequired =
    Number.isFinite(semanticCheckedAt) &&
    semanticCheckedAt >= Date.parse(EDITORIAL_SEMANTIC_QC_CUTOVER);
  if (semanticRequired) {
    add('main_claim_visualization', Number(qa.main_claim_visualization || 0) >= 8, '記事の主張を画像で十分に可視化できていません。');
    add('article_theme_inferable_without_title', boolIs(qa.article_theme_inferable_without_title, true), 'タイトルを隠すと記事テーマが読み取れません。');
    add('scene_action_article_specific', boolIs(qa.scene_action_has_article_specific_meaning, true), '人物の行動が記事固有の意味を持っていません。');
    add('generic_passive_pose_absent', boolIs(qa.generic_passive_pose_without_article_reason, false), '記事上の理由がない受動的な人物配置です。');
  } else {
    checks.semantic_qc_legacy_pre_cutover = true;
  }
  ...
  return {
    ...
    legacyAcceptedReference,
    realismRequired,
    realismQcVersion: realismRequired ? EDITORIAL_REALISM_QC_VERSION : null
  };
}
