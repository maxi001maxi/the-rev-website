...
export function evaluateEditorialImageReview({ draft = {}, qa = draft?.image_qa || {}, recentHistory = [] } = {}) {
  ...
  if (realismRequired) {
    ...
    add('human_environment_integration', Number(qa.human_environment_integration || 0) >= 9, '人物と実背景の統合品質が9未満、または未記録です。');
    add('perspective_scale_consistency', Number(qa.perspective_scale_consistency || 0) >= 9, '人物の縮尺・遠近整合が9未満、または未記録です。');
    add('ground_contact_shadow_consistency', Number(qa.ground_contact_shadow_consistency || 0) >= 9, '床接地・接触影の整合が9未満、または未記録です。');
    add('lighting_consistency', Number(qa.lighting_consistency || 0) >= 9, '人物と背景の光・色温度整合が9未満、または未記録です。');
    add('anatomy_pose_realism', Number(qa.anatomy_pose_realism || 0) >= 9, '人物の解剖・姿勢リアリズムが9未満、または未記録です。');
