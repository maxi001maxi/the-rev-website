...
export const CONTENT_REFERENCE_SELECTION_POLICY = 'relevance-first-recency-second-v1';
...
function sourceCandidates(article) {
  ...
  if (category === 'boxing') {
    return [pick('assets/images/photo-boxing.jpg', 'boxing-action', 'ボクシング記事に実際のミット・指導シーンを対応', 'primary')];
  }
  ...
  if (/時間がない|通い|通う|仕事帰り|準備|移動|着替え|帰宅|ルーティン|習慣|出発/.test(text)) {
    return [
      ...
      pick('assets/images/trainer-top.jpg', 'arrival-guidance', '来店・継続テーマを人物の存在感で補完', 'secondary'),
      pick('assets/images/trainer-coaching.jpg', 'state-check-coaching', '個別調整の文脈をコーチング実写で表現', 'secondary')
    ];
  }

  if (/疲れ|疲労|仕事終わり|だる|眠|休む|休息|コンディション|調子|判断|軽く始め|無理|モチベーション/.test(text)) {
    return [
      pick('assets/images/trainer-coaching.jpg', 'state-check-coaching', '疲労・当日の状態判断をトレーナーと状態を確認する実写で表現', 'primary'),
      pick('assets/images/photo-lobby.jpg', 'state-check-arrival', '疲労日の来店判断を生活動線の実写で補完', 'secondary'),
      pick('assets/images/photo-evolgear.jpg', 'light-training-space', '軽く始める選択肢をトレーニング空間で表現', 'secondary')
    ];
  }

  if (/フォーム|姿勢|動作|可動|指導|初心者|パーソナル|カウンセリング|体験|肩|腰|膝/.test(text)) {
    return [
      pick('assets/images/trainer-coaching.jpg', 'coaching-form', 'フォーム・姿勢・初心者・個別指導の内容にコーチング実写を対応', 'primary'),
      pick('assets/images/trainer-top.jpg', 'trainer-guidance', '指導者の存在を維持できる別実写候補', 'secondary'),
      pick('assets/images/photo-evolgear.jpg', 'training-context', '指導が行われる実トレーニング環境を補完', 'fallback')
    ];
  }
  ...
  if (/ラック|マシン|器具|設備|evolgear|重量|負荷|筋力|ベンチ|スクワット|デッドリフト/.test(text)) {
    return [
      ...
      pick('assets/images/trainer-top.jpg', 'training-guidance', '設備テーマを人の利用文脈で補完', 'secondary'),
      pick('assets/images/trainer-coaching.jpg', 'coaching-context', '設備を使う個別指導の文脈で補完', 'fallback')
    ];
  }
  ...
  if (category === 'training') {
    return [
      ...
      pick('assets/images/trainer-top.jpg', 'training-guidance', 'トレーニング文脈を人物実写で補完', 'secondary'),
      pick('assets/images/photo-lobby.jpg', 'training-arrival', '継続・来店文脈を実施設で補完', 'secondary')
    ];
  }

  return [
    pick('assets/images/trainer-coaching.jpg', 'body-knowledge-coaching', '身体知識の記事は人の動き・指導との接続を優先', 'primary'),
    pick('assets/images/trainer-top.jpg', 'body-knowledge-trainer', '人物中心の身体知識文脈を維持できる別実写候補', 'primary'),
    pick('assets/images/photo-lobby.jpg', 'body-knowledge-lifestyle', '身体知識を日常・来店文脈へ接続', 'secondary'),
    pick('assets/images/photo-evolgear.jpg', 'body-knowledge-training', '身体知識を実トレーニング環境へ接続', 'secondary')
  ];
}
...
export function selectContentReferenceWithHistory(article, recentHistory = []) {
  ...
  // 正本仕様:
  // 1. 記事との関連性を最優先する。
  // 2. 同じ関連性レベルの候補内で、直近4記事で未使用の写真を優先する。
  // 3. 同レベル候補が全て使用済みなら、最も古く使われた写真を選ぶ。
  // 4. 関連性が一段落ちる写真へは、重複回避だけを理由に降格しない。
  ...
  const selected = [...relevantPool].sort((a, b) => {
    ...
    if (da !== db) return db - da; // unused first; otherwise least-recently used
  ...
  const repeatedDueToRelevance =
    selectedRecentIndex >= 0 &&
    !relevantPool.some((x) => recencyIndex(x.path) < 0);
...
export function buildEditorialImagePlan(article, { sourceDecisionOverride = null, recentHistory = [] } = {}) {
  ...
  const sourceDecision = explicitSource
    ...
    : (sourceDecisionOverride || selectContentReferenceWithHistory(article, recentHistory));
  ...
  const job = {
    ...
    recent_reference_guard: {
      ...
      priority_order: ['article_relevance', 'not_used_in_recent_4', 'least_recently_used'],
...
export async function prepareEditorialImageJob(article) {
  ...
  const sourceDecision = clean(article?.sourceImage)
    ? null
    : selectContentReferenceWithHistory(article, recentHistory);
