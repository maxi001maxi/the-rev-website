...
function sourceCandidates(article) {
  ...
  if (/疲れ|疲労|仕事終わり|だる|眠|休む|休息|コンディション|調子|判断|軽く始め|無理|モチベーション/.test(text)) {
    return [
      ...
      pick('assets/images/trainer-top.jpg', 'state-check-guidance', '疲労・当日の状態判断を、利用者と会話しながら状態を見る実写で表現', 'primary'),
...
export async function checkEditorialImageReady(draft, { env = process.env } = {}) {
  ...
  const hybridQaReady =
    qa?.pass === true &&
    String(draft?.image_render_version || qa?.render_version || '') === 'rev-column-reference-v2.3-hybrid' &&
    String(draft?.image_strategy || '') === 'reference-v2-gpt-image-hybrid-drive-source' &&
    Number(qa?.editorial_quality || 0) >= 8 &&
    Number(qa?.rev_environment_consistency || 0) >= 8 &&
    Number(qa?.brand_space_authenticity || 0) >= 8 &&
  ...
  if (!qa || (!sourceLockQaReady && !hybridQaReady)) {
    return {
      ready: false,
