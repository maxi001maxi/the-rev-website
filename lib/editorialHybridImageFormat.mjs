...
export const SEMANTIC_QC_CUTOVER = '2026-09-26T00:00:00.000Z';
...
export const HYBRID_IMAGE_FORMAT = Object.freeze({
  ...
  activationMode: 'github-actions-auto-operator-v1',
  ...
  output: Object.freeze({
    ...
    gbp: Object.freeze({ width: 1200, height: 900, ratio: '4:3' })
  }),
  ...
  generationPolicy: Object.freeze({
    ...
    sceneAwareIntegrationRequiredForExercise: true,
    isolatedPersonCutoutCompositeForbiddenForExercise: true,
    preferredHighMotionModelClass: 'FLUX Kontext/Pro or equivalent scene-aware photoreal editor',
    unavailableHighMotionProviderPolicy: 'simplify scene or keep PREPARING; never silently accept a cutout composite',
    ...
    rule: 'THE REV.の実素材を空間の正本として使う。人物生成は顧客役のみ許可し、未知のトレーナー・スタッフ・第三者は生成しない。背景は実素材の構造がTHE REV.として認識できる範囲でぼかし・柔らかさ・奥行き調整を許可する。全身のトレーニング動作を入れる場合は人物切り抜き貼付ではなくscene-awareな生成/編集を必須とし、縮尺・遠近・床接地・影・光・器具との接触が自然に成立しない画像は採用しない。高難度動作はFLUX Kontext/Pro等のscene-aware photoreal editorを優先し、利用できない場合は静的な場面へ簡略化するかPREPARINGで止める。'
  }),
  designGrammar: Object.freeze({
    ...
    requirements: Object.freeze([
      ...
      '写真の存在感と文字の可読性を両立する',
      '全身運動を描く場合、受付・通路ではなく実際のトレーニングエリアとして意味が通る位置に人物を置く',
      '人物の縮尺、カメラ遠近、足裏の床接地、接触影、光源方向、器具との接触を一枚の写真として自然に一致させる',
      '人物だけを後貼りしたステッカー/切り抜き合成に見える画像は禁止する'
    ])
  }),
  qc: Object.freeze({
    ...
    minMainClaimVisualization: 8,
    requireArticleThemeInferableWithoutTitle: true,
    requireSceneActionArticleSpecific: true,
    requireGenericPassivePoseWithoutArticleReasonFalse: true,
    minRevEnvironmentConsistency: 8,
    minBrandSpaceAuthenticity: 8,
    realismQcVersion: 'v1',
    minHumanEnvironmentIntegration: 8,
    minPerspectiveScaleConsistency: 8,
    minGroundContactShadowConsistency: 8,
    minLightingConsistency: 8,
    minAnatomyPoseRealism: 8,
    requireNoCutoutOrStickerLook: true,
    requireLocationSemanticsPass: true,
    requireExercisePosePlausible: true,
    requireManualVisualRejectionFalse: true,
...
export function hybridQaReady(draft, qa = draft?.image_qa) {
  ...
  return (
    ...
    (
      !(
        Number.isFinite(Date.parse(clean(qa.checked_at))) &&
        Date.parse(clean(qa.checked_at)) >= Date.parse(SEMANTIC_QC_CUTOVER)
      ) ||
      (
        Number(qa.main_claim_visualization || 0) >= HYBRID_IMAGE_FORMAT.qc.minMainClaimVisualization &&
        qa.article_theme_inferable_without_title === HYBRID_IMAGE_FORMAT.qc.requireArticleThemeInferableWithoutTitle &&
        qa.scene_action_has_article_specific_meaning === HYBRID_IMAGE_FORMAT.qc.requireSceneActionArticleSpecific &&
    ...
    (
      clean(qa.realism_qc_version) !== HYBRID_IMAGE_FORMAT.qc.realismQcVersion ||
      (
        Number(qa.human_environment_integration || 0) >= HYBRID_IMAGE_FORMAT.qc.minHumanEnvironmentIntegration &&
        Number(qa.perspective_scale_consistency || 0) >= HYBRID_IMAGE_FORMAT.qc.minPerspectiveScaleConsistency &&
        Number(qa.ground_contact_shadow_consistency || 0) >= HYBRID_IMAGE_FORMAT.qc.minGroundContactShadowConsistency &&
        Number(qa.lighting_consistency || 0) >= HYBRID_IMAGE_FORMAT.qc.minLightingConsistency &&
        Number(qa.anatomy_pose_realism || 0) >= HYBRID_IMAGE_FORMAT.qc.minAnatomyPoseRealism &&
        qa.no_cutout_or_sticker_look === HYBRID_IMAGE_FORMAT.qc.requireNoCutoutOrStickerLook &&
        qa.location_semantics_pass === HYBRID_IMAGE_FORMAT.qc.requireLocationSemanticsPass &&
        qa.exercise_pose_plausible === HYBRID_IMAGE_FORMAT.qc.requireExercisePosePlausible
      )
    ) &&
    ...
    (
      !clean(draft?.gbp_image_asset_version) ||
      (
...
export function hybridImageInfoFromDraft(draft) {
  ...
  return {
    ...
    gbpImage: draft.gbp_image || null,
    gbpImageStatus: draft.gbp_image_status || null,
    gbpImageAssetVersion: draft.gbp_image_asset_version || null,
    gbpImageQa: draft.gbp_image_qa || null,
...
export function hybridAssetPaths(slug, assetVersion) {
  ...
  return {
    ...
    gbpRepoPath: `assets/images/gbp/gbp-${safeSlug}-${safeVersion}.jpg`,
    ...
    gbpPublicPath: `/assets/images/gbp/gbp-${safeSlug}-${safeVersion}.jpg`
  };
}
...
export function buildHybridImageJob({
  ...
  return {
    ...
    gbp_image: paths.gbpRepoPath,
    gbp_image_required: true,
    gbp_image_width: HYBRID_IMAGE_FORMAT.output.gbp.width,
    gbp_image_height: HYBRID_IMAGE_FORMAT.output.gbp.height,
    gbp_image_aspect_ratio: HYBRID_IMAGE_FORMAT.output.gbp.ratio,
    ...
    policy: {
      ...
      gbp_image_required: true,
      gbp_image_ratio: HYBRID_IMAGE_FORMAT.output.gbp.ratio,
      layout_template_id: HYBRID_IMAGE_FORMAT.layoutTemplateId,
      policy_revision: HYBRID_IMAGE_FORMAT.policyRevision,
      ...
      scene_aware_integration_required_for_exercise: HYBRID_IMAGE_FORMAT.generationPolicy.sceneAwareIntegrationRequiredForExercise,
      isolated_person_cutout_composite_forbidden_for_exercise: HYBRID_IMAGE_FORMAT.generationPolicy.isolatedPersonCutoutCompositeForbiddenForExercise,
      preferred_high_motion_model_class: HYBRID_IMAGE_FORMAT.generationPolicy.preferredHighMotionModelClass,
      unavailable_high_motion_provider_policy: HYBRID_IMAGE_FORMAT.generationPolicy.unavailableHighMotionProviderPolicy,
      realism_qc_version: HYBRID_IMAGE_FORMAT.qc.realismQcVersion,
...
export function hybridGenerationBrief({
  ...
  return [
    ...
    '- 全身のトレーニング動作は人物だけを切り抜いて後貼りしない。背景を含むscene-aware編集/生成で、縮尺・遠近・床接地・影・光・器具接触を一体で成立させる。',
    '- 高難度の運動動作はFLUX Kontext/Pro等のscene-aware photoreal editorを優先する。利用できない場合は、セット直後に自然に立つ・ベンチで休む等の低難度シーンへ簡略化するか、画像をPREPARINGのまま止める。',
    '- 受付や通路でスクワット等、店舗の使われ方として不自然な配置は禁止する。',
